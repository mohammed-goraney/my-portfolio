            ...
            >
              <Card variant="bordered" padding="lg" className="flex flex-col h-full">
                <div className="flex items-start justify-between mb-6">
                  ...
                  <div className="ml-4 text-right">
                    ...
                    <p className="text-caption text-text-muted">
                      {category.yearsOfExperience ?? 0}+ yrs
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full bg-surface-light rounded-full h-2 mb-6 overflow-hidden">
                  <div
                    className={`h-full bg-gradient-to-r ${getSkillColor(category.level ?? 0)} transition-all duration-500`}
                    style={{ width: `${category.level ?? 0}%` }}
                  />
                </div>
                  ...
                  {category.skills?.map((skill: typeof category.skills[0]) => (
                    <div key={skill.name} className="flex items-start justify-between">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        {skill.verified && (
                          <span title="Verified skill" className="text-accent-gold">
                            ✓
                          </span>
                        )}
                      </div>
